...
<script type="text/javascript">
	jQuery(document).ready(function($){
		function sendPageTranslateMail(form){

			var url = $(form).find("#pt_url").val();
			var email = $(form).find("#pt_email").val();
			var name = $(form).find("#pt_name").val();

			console.log(url + ' ' + email + ' ' + name);
			...
			if ( send ) {
				$.ajax({
				    type     : "POST",
				    url: 	'/api/web/index.php/sendMailPageTranslate',
				    data     : {
				    		url : url,
				    		email : email,
				            name : name
		...
		$('#page-translation-order input').keypress(function (e) {
			var key = e.which;
			if(key == 13){
				$("#page-translation-submit").click();
				return false;  
			}
		}); 

		$("#page-translation-submit").click(function(){
			var form = $("#page-translation-order");
			sendPageTranslateMail(form);
		});
	});
</script>
...
<div class="landing_sections" style="margin: 0 auto;">
	...
	<div class="row">
		<div class="col-md-4 section-title">
			...
			<?php endif; ?>
			<img src="<?php  the_sub_field('lp_section_icon') ?>" alt="" style="max-width:200px;">
		</div>
...
<div class="row pt-orderform" style="top:0;">

	<div id="pt-summary" class="col-md-4" style="top:43px">

		<div id="pt-error-response" class="text-center">
            <h3>Error in sending mail</h3>
            <p>Please <a href="mailto:info@translatedbyus.com">contact us</a>.</p>
        </div>
		
		<div id="pt-response" class="text-center" style="margin-top:14px;">
            <?php the_field('pt_order_confirmation_text'); ?>
        </div>
	...
	<div id="pt-orderformwrapper" class="col-md-8" style="padding-top:0; padding-bottom:0;">
	
		<form name="page-translation-order" id="page-translation-order" action="#" method="POST">
			
			<h2><?php _e('Fill in your details', 'TBU'); ?></h2> 
			<section class="order-block">
			
				<div class="row">
					<div class="col-md-4"><input type="text" name="ClientUrl" placeholder="* <?php _e('URL', 'TBU'); ?>" id="pt_url" class="required" /></div>
					<div class="col-md-4"><input type="email" name="ClientEmail" placeholder="* <?php _e('Email', 'TBU'); ?>" id="pt_email" class="required" /></div>
					<div class="col-md-4"><input type="text" name="ClientName" placeholder="<?php _e('Name', 'TBU'); ?>" id="pt_name" /></div>
				</div>
				<input type="button" id="page-translation-submit" class="btn disabledz" value="<?php if(get_bloginfo('language') === 'da-DK'): echo "Start ordtælling"; else: _e('Send order', 'TBU'); endif; ?>"/>                      
			</section>
		...
		<?php 
			$after_page_translate_form = get_field('pt_after_order_text'); 
			if( $after_page_translate_form != ''):
			?>
			<div class="after-form">
				<?php echo $after_page_translate_form; ?>
			</div>
			<?php 
			endif; 
		?>
